...
ggplot(sat_adm_data_high, 
       aes(x = sat_adm_data_high$SAT_AVG, 
           y = sat_adm_data_high$ADM_RATE, 
           ...
           theme(legend.position = "none", 
                 ...
                 strip.background = element_rect(fill = "black"),
                 strip.text = element_text(size = 8, colour = "white")) +
           coord_cartesian(xlim=c(1200, 1700), ylim=c(0,0.30)) + 
           geom_label_repel(aes(x = sat_adm_data_high$SAT_AVG,
                                y = sat_adm_data_high$ADM_RATE,
                                fill = factor(sat_adm_data_high$STABBR),
                                label = sat_adm_data_high$STABBR),
                            size = 2.6, force = 1, fontface = "bold", 
                            colour = "black")
```

Since the data is much less dense, we can add labels. It seems like Massachusetts, California, and New York are some of the more recurring data points. This makes sense, since these states are home to, not just one or two, but multiple prestigious schools. The plots also increase in density as time goes on, which seems to indicate that students (and entire schools as a result) are becoming smarter on average. Of course, intelligence is measured by more than just standardized testing, but this is a pretty uplifting result.
